...
void parse(char *str)
{
    ...
    while (right <= len && left <= right)
    {
        ...
        if (isDelimiter(str[right]) == true && left == right)
        {
            ...
            {
                count++;
                printf("%-15c an operator\n", str[right]);
            }
            else if (isSpecial(str[right]) == true)
            {
                count++;
                printf("%-15c a special symbol\n", str[right]);
            }
            right++;
            left = right;
        }
        else if (isDelimiter(str[right]) == true && left != right || (right == len && left != right))
        {
            ...
            {
                count++;
                printf("%-15s a keyword\n", subStr);
            }
            else if (isInteger(subStr) == true)
            {
                count++;
                printf("%-15s an integer\n", subStr);
            }
            else if (isRealNumber(subStr) == true)
            {
                count++;
                printf("%-15s a real number\n", subStr);
            }
            else if (validIdentifier(subStr) == true && isDelimiter(str[right - 1]) == false)
            {
                count++;
                printf("%-15s an identifier\n", subStr);
            }
            else if (validIdentifier(subStr) == false && isDelimiter(str[right - 1]) == false)
            {
                count++;
                printf("%-15s an invalid identifier\n", subStr);
            }
            left = right;
        }
    }
